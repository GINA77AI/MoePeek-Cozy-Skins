...
/// Reads visual parameters from `ThemeManager.shared.current`.
@MainActor
final class TriggerTrackingView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        ctx.setShadow(
            ...
            blur: t.triggerGlowBlur,
            color: t.triggerGlowColor.withAlphaComponent(CGFloat(t.triggerGlowOpacity)).cgColor
        )
        ctx.setFillColor(t.triggerBaseColor.cgColor)
        ...
        let gradient = CGGradient(
            ...
            colors: [
                NSColor.white.withAlphaComponent(CGFloat(t.triggerInnerGradientTopAlpha)).cgColor,
        ...
        // Border
        ctx.setStrokeColor(t.triggerBorderColor.withAlphaComponent(CGFloat(t.triggerBorderOpacity)).cgColor)
        ctx.setLineWidth(0.75)
        ctx.strokeEllipse(in: rect)

        // SF Symbol icon — theme-colored
        let sizeConfig = NSImage.SymbolConfiguration(pointSize: 14, weight: .medium)
        let colorConfig = NSImage.SymbolConfiguration(hierarchicalColor: t.triggerIconColor)
