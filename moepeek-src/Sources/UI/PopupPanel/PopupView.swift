    ...
    var body: some View {
        ...
        .background(
            ZStack {
                ...
                // Base color
                t.panelBaseColor.opacity(0.92)
                // Gradient overlay for depth
                if let bottom = t.panelGradientBottom {
                    LinearGradient(
                        colors: [
                            t.panelGradientTop,
                            t.panelGradientMid,
        ...
        // Shadow & corner — theme-driven
        .shadow(color: ThemeManager.shared.current.panelShadowColor,
                radius: ThemeManager.shared.current.panelShadowRadius, y: 6)
        .clipShape(RoundedRectangle(cornerRadius: ThemeManager.shared.current.panelCornerRadius))
